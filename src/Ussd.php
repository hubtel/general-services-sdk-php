...
class Ussd
{
     ...
     * \com\HubtelUssdFramework\demo\controllers\MainController can have its
     * namespace specified as
     * <ol>
     *  <li>\com
     *  <li>\com\smsgh
     *  <li>\com\HubtelUssdFramework
     *  <li>\com.HubtelUssdFramework\demo
     *  <li>\com\HubtelUssdFramework\demo\controllers
     * </ol>
     * 
     ...
     * \com\HubtelUssdFramework\demo\controllers\MainController can be specified as
     * <ol>
     *  <li>\com\HubtelUssdFramework\demo\controllers\MainController
     *  <li>\HubtelUssdFramework\demo\controllers\Main
    ...
    function doOptions()
    {
        ...
        header('Access-Control-Allow-Origin: ' . ($this->_accessControlAllowOrigin ?
                $this->_accessControlAllowOrigin : self::DEFAULT_ACCESS_CONTROL_ALLOW_ORIGIN));
    ...
    function processRequest($request)
    {
        ...
        try {
            ...
            $response->setType(UssdResponse::RESPONSE_TYPE_RELEASE);
            $this->logResponse($response);
            if ($context) {
                $context->sessionClose();
                $context->close();
    ...
    private function processInitiationRequest($context, $route)
    {
        ...
        if (
            $this->_maxAutoDialDepth > 0 && $ussdResponse->isAutoDialOn() &&
    ...
    public static function process($logPath, $redisStore, $ussdControllerNameSpace, $initiationController, $initiationAction, $accessControlAllowOrigin)
    {

        Loggers::setDebugLogger($logPath);
        Loggers::setErrorLogger($logPath);
        ...
        // $ussd->store(new UssdFramework\Stores\RedisStore1());
        $ussd->store($redisStore);
        // }

        $ussd->controllerNamespaces(array($ussdControllerNameSpace))
            ->initiationController($initiationController)
            ->initiationAction($initiationAction)
