...
class UssdController {
     ...
     * @var string
     */
    const MENU_PROCESSOR_DATA_KEY = 'HubtelUssdFramework\UssdController\MenuProcessorData';
    
    /**
     * @var string
     */
    const FORM_PROCESSOR_DATA_KEY = 'HubtelUssdFramework\UssdController\FormProcessorData';
    
    /**
     * @var string
     */
    const FORM_DATA_KEY = 'HubtelUssdFramework\UssdController\FormData';
     ...
     */
    function render($message, $action = null, $controller = null, $autoDialOn = true, $listArgs = null) {
        // if ( ! $message) {
        //     $message = '';
        // }
        // $route = null;
        ...
        // return $ussdResponse;
        return !$listArgs ?  
            $this->renderMessage($message, $action , $controller, $autoDialOn)
           : $this->renderList($message, $action, $controller , $autoDialOn , $listArgs);

    }

    //additions 
    function renderMessage($message, $action = null, $controller = null, $autoDialOn = true){
        if (!$message) {
            $message = '';
        }
        $route = null;
    ...
    function renderList($message, $action, $controller, $autoDialOn, $listArgs){
        if (!$message) {
            $message = '';
        }
        $route = null;
        ...
        $ussdResponse = UssdResponse::render($message, $route, $listArgs['dataItems'], $listArgs['cartItem'], $listArgs['label'], $listArgs['dataType'], 
            $listArgs['fieldType'], $listArgs['sessionId'],
            $listArgs['mask'],
            $listArgs['maskNextRoute'],
            $listArgs['serviceCode']);
        $ussdResponse->setAutoDialOn($autoDialOn);
        return $ussdResponse;
    }
     ...
     */
    function renderMenu($ussdMenu, $autoDialOn = true, $listArgs = null) {
        // if ( ! $ussdMenu) {
        ...
        return  !$listArgs ? $this->renderMenuWithMessage($ussdMenu, $autoDialOn) : $this->renderMenuWithList($ussdMenu, $autoDialOn, $listArgs);
    }

    //additions
    function renderMenuWithMessage($ussdMenu, $autoDialOn){
        if (!$ussdMenu) {
            throw new \InvalidArgumentException('"ussdMenu" argument cannot '
                . 'be null');
        }
    ...
    function renderMenuWithList($ussdMenu, $autoDialOn, $listArgs){
        if (!$ussdMenu) {
            throw new \InvalidArgumentException('"ussdMenu" argument cannot '
                . 'be null');
        }
        ...
        $this->_dataBag->set(self::MENU_PROCESSOR_DATA_KEY, $repr);
        $message = $ussdMenu->render();
        return $this->render($message, 'menuProcessor', null, $autoDialOn, $listArgs);
    }
     ...
     */
    function renderForm($form, $autoDialOn = true, $listArgs = null) {
        return !$listArgs ? $this->renderFormWithMessage($form, $autoDialOn) : $this->renderFormWithList($form, $autoDialOn, $listArgs);
        // if ( ! $form) {
    ...
    function renderFormWithMessage($form, $autoDialOn = true)
    {
        if (!$form) {
            throw new \InvalidArgumentException('"form" argument cannot '
                . 'be null');
    ...
    function renderFormWithList($form, $autoDialOn = true, $listArgs = null)
    {
        if (!$form) {
            throw new \InvalidArgumentException('"form" argument cannot '
                . 'be null');
        ...
        $this->_dataBag->set(self::FORM_PROCESSOR_DATA_KEY, $repr);
        $message = $form->render();
        return $this->render($message, 'formProcessor', null, $autoDialOn, $listArgs);
    }
